...
```{r}
mi_est1$outcome$single_shift$coefficients
```


DR selection and outcome is exactly the same as it is based on the joint estimation

```{r}
dr_est2$selection$single_shift$x
```
...
```{r}
hist(weights(dr_est1), breaks = "fd")
```
