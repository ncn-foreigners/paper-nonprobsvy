...
<!--- Piotrek, 17.11.2024:
Co się stało z tą tabelą? Nie można przekleić tej z pmm? tam wyglądała dużo lepiej
-->
...
<!--- Piotrek, 17.11.2024:
Ta tabela to bardziej set-up niż założenie,
-->
...
As you can see, three new arguments have appeared - \code{selection} and \code{target} are responsible for the formulas for modelling the inclusion model and for defining the variable for which we calculate the population mean. In addition, in the \code{method\_selection} argument, we specify the name of the link function that models the probability of inclusion in the non-probability sample. For the \textbf{doubly robust estimator} call it is as follows
...
In this case the \code{target} is not needed as we define \code{selection} and \code{outcome} arguments. We also provide details on mass imputation and propensity score models to obtain a doubly robust estimator. Importantly, arguments such as \code{method\_outcome}, \code{method\_selection} or \code{family\_outcome} (and a few others) take default values described in more detail in the package documentation.
...
Mass imputation methods are defined in the argument \code{method\_outcome}. In the control function, we can set the parameters of a given method, e.g., the number of nearest neighbours in the NN algorithm.
...
The result of the function call will be an object of the class \pkg{nonprobsvy} containing a list of elements related to the estimation, i.e. the value of the estimated parameters, the mean and its standard deviation. On such an object, we can call the \code{summary} method, familiar to users of the R language. The result will look like this
