...
\end{itemize}
<!----To nie tyle że zakładamy że tego overlapu nie ma tylko po prostu nie bierzemy go pod uwagę, to nie pogarsza estymatora tylko nie możemy go poprawić.---->
...
Inverse probability weighting (IPW) is another popular estimation approach, involves estimating PS given by $\pi_i^A=P\left(i \in S_A \right)$. As in the case of the prediction-based approach, there are two variants of the IPW estimator, given by:
...
then solving for $\bG(\bgamma)=\bZero$ with the chosen parametric form of $\pi_i^A$ and the chosen $\bh(\bx,\bgamma)$ produces the consistent estimator of $\hat{\bgamma}$.  In the literature, the most commonly considered functions are $\bh\left(\bx_i, \bgamma\right) = \bx_i$ and $\bh\left(\bx_i, \bgamma\right) = \bx_i \pi\left(\bx_i, \bgamma\right)^{-1}$.  Note that if the function $\bh_i=\bx_i$, then $\bG$ reduces to

$$
\bG(\bgamma) = \sum_{i \in S_A} \bx_i-\sum_{i \in S_B} d_i^B \pi\left(\bx_i, \bgamma\right) \bx_i,
$$

and for the second variant of the $\bh$ function we get the following form of function $\bG$
