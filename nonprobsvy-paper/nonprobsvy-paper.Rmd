...
In addition, we assume no overlap between $S_A$ and $S_B$, and no measurement error in $Y_i$ and $\bX_i$ is observed. Setting presented in Table \ref{tab-two-sources} may be also extended to calibrated $d_i^B$ weights (i.e. $d_i^B$ adjusted for under-coverage, non-contact or non-response; cf. \citet{sarndal2005estimation}) but this requires additional developments in the theory about the consistency of the MI, IPW or DR estimators. In the next sections we briefly present methods that are implemented in the package.

## Prediction-based approach {#sec-prediction}

### General form of prediction estimators

In prediction approach the following semiparametric model for the finite population, denoted as $\xi$ is assumed
...
\begin{equation}
\hat{\mu}_{y 1}=\mu_{\bx}^{\prime} \hat{\bbeta} \quad \text { and } \quad \hat{\mu}_{y 2}=\frac{n_A}{N}\left(\bar{y}_A-\overline{\bx}_A^{\prime} \hat{\bbeta}\right)+\mu_{\bx}^{\prime} \hat{\bbeta},
\label{eq-pred-two-estimators-simplified}
\end{equation}

where $\mu_{\bx} = N^{-1}\sum_{i=1}^N\bx_i$ is the vector of the population means of the $\bx$ variables and $\overline{\bx}_A=n_A\sum_{i \in S_A}\bx_i$ is the vector of the simple means of $\bx$ from the non-probability sample $S_A$. If the linear model contains an intercept and $\hat{\bbeta}$ is the ordinary least square estimator then $\hat{\mu}_{y 1}=\hat{\mu}_{y 2}$. 

This form is appealing as the only requirement is availability of non-probability sample $S_A$ and reference population means (or totals and population size $N$). If the population means are unknown they can be replaced by estimates provided by the reference probability sample $S_B$: $\sum_{i=1}^N \hat{m}_i = \sum_{i \in S_B} d_i^B\hat{m}_i$ for \eqref{eq-pred-two-estimators} and $\mu_{\bx}$ replaced by $\hat{\mu}_{\bx}=\hat{N}_B^{-1}\sum_{i \in S_B}d_i^B\bx_i$ for \eqref{eq-pred-two-estimators-simplified} where $\hat{N}_B=\sum_{i \in S_B}d_i^B$. 


### Mass imputation estimators

Model-based prediction estimators of $\mu_y$ can be treated as \textit{mass imputation estimators} as the information on $y_i$ is missing entirely in the reference probability sample $S_B$ (but $\bx_i$ is available) and $y_i$ can be imputed based on the non-probability sample $S_A$ as $(y_i, \bx_i)$ is observed. The general for of the mass imputation (MI) estimator is given by 

\begin{equation}
\hat{\mu}_{y MI}=\frac{1}{\hat{N}_{B}} \sum_{i \in S_{B}} d_i^{B} y_i^*, 
\end{equation}

where $y_i^*$ is the imputed $y_i$ value and $\hat{N}_{B}$ is defined previously. Under deterministic regression imputation the $\hat{\mu}_{y MI}$ estimator reduce to \eqref{eq-pred-two-estimators} estimators.

There are several approaches how to impute $y_i^*$ and in the package we have implemented the following MI estimators: semiparametric approach based on the generalized linear models (hereinafter denoted as MI-GLM), nearest neighbour matching (denoted as MI-NN) and predictive mean matching (MI-PMM).

The properties of the MI-GLM estimator was studied by \citet{kim_combining_2021} additional assumption called \textit{model transportability}: the model which is built for the non-probability sample $S_A$ can be used for prediction with the reference probability sample $S_B$. In the package we allow for the following GLM families: \code{gaussian}, \code{binomial} and \code{poisson}. 

The MI-NN estimator was initially proposed by \citet{rivers2007sampling} under the name \textit{sample matching} and theoretical properties for the MI-NN estimator for large non-probability samples (i.e. $N n_A^{-1}=O(1)$) was studied by \citet{yang2021integration}. The basic idea of the NN matching is as follows: 1) for each $i$ unit in the probability sample $S_B$ find a donor $j$ (or donors) in sample $S_A$ based on some distance between $\bx_i$ and $\bx_j$; 2) use the matched values $y_j$ from $S_A$ to impute missing $y_i$ values in the probability sample $S_B$. Imputed values $y_i^*$ depends on the number of selected neighbours. For $k=1$ the closest one is selected and for $k>1$ a simple average over a vector of selected $y$ is calculated. The detailed description is presented in the Algorithm \ref{algo-2} in the Appendix. This algorithm suffers from the curse of dimensionality as proved by for i.e. asymptotic bias of the MI estimator increases as the number of covariates $\bx$ increases with a fixed $k$. To overcome this issue PMM was proposed

In the PMM matching is done using predicted values of $\hat{m}_i=m\left(\bx_i, \hat{\bbeta}\right)$ instead of $\bx_i$, thus the NN algorithm is modified as follows: 1) fit the $m\left(\bx_i, \bbeta\right)$ to non-probability sample $S_A$, 2) assign predicted values $\hat{m}_i$ to all units in $S_A$ and $S_B$; 3) match all units from sample $S_B$ to donor units from sample $S_A$. The remaining part is the same as in the NN approach. \citet{chlebicki2025} studied properties of two variants of the MI-PMM estimator for non-probability samples: matching predicted to predicted ($\hat{m}-\hat{m}$ matching; MI-PMM A) and predicted to observed ($\hat{m}-y$ matching; PMM B). Details about the procedure can be found in Algorithm \ref{algo-3} and  \ref{algo-4} in the Appendix. \citet{chlebicki2025} prove consistency of the MI-PMM A estimator under model mis-specification i.e. the assumed model may be different from the true one. 


### Variance estimators for the prediction approach 

Variance for the MI estimators can be either estimated using analytical or bootstrap approach. Analytical estimator of the variance of the MI-GLM estimator proposed by \citet{kim_combining_2021} (p. 950) which contains two components: $\hat{V}_1$ (based on information from both $S_A$ and $S_B$) and $\hat{V}_2$ (based solely on probability sample $S_B$); for the MI-NN estimator for large samples \citet{yang2021integration} proposed variance estimator based solely on the probability sample $S_B$ (i.e. design-based variance estimator of the mean) and for small $S_A$ samples variance estimator can be found in \citet{chlebicki2025}; and for the MI-PMM estimators \citet{chlebicki2025} contains formulas for variance estimators for small and large non-probability samples $S_A$. 

Second approach is based on the bootstrap where each run of the procedure $b=1,...,B$ can be expresed as follows

\begin{enumerate}
\item Independently:
  \begin{itemize}
  \item draw a simple random sampling with replacement from the non-probability sample $S_A$,
  \item draw a sample according to the declared sampling design from the probability sample $S_B$.
  \end{itemize}
\item Estimate $\mu_{y MI}^b$ based on appropriate approach (e.g. MI-GLM, MI-NN or MI-PMM).
\end{enumerate}

After obtaining $B$ bootstrap estimate estimate variance based on the following equation
...
where $\hat{\mu}_{y}$ is the estimated mean using either MI-GLM, MI-NN or MI-PMM estimator.

The above approaches are applied when unit-level data from the probability sample $S_B$ are available. If this is not the case and only population means (or totals and population size) are available we use the first component $\hat{V}_1$ of the \citet{kim_combining_2021} variance estimator (estimated population quantities are replaced with known ones). For the variance of the MI-NN and MI-PMM estimators we only use bootstrap approach with known population means. Note, that current version of the \pkg{nonprobsvy} does not support the replicated weights in the probability sample $S_B$ for any of the estimators discussed in this section.

## Inverse Probability Weighting {#sec-ipw}

Another popular approach is inverse probability weighting (IPW) that involves estimation of propensity scores denoted as $\pi_i^A=P\left(i \in S_A \right)$ which denotes inclusion and participation probabilities. Similarly as for the prediction approach we have two variants of the IPW estimators given by 
...
where the $\hat{\mu}_{IPW1}$ is a version of the Horvitz-Thompson estimator and the $\hat{\mu}_{IPW2}$ is the Hájek estimator with the estimated population size given by $\hat{N}^A = \sum_{i \in S_A} \frac{1}{\pi_i^A}$. This estimator in the case of non-probability samples was discussed by .. and several approaches on using propensity scores and alternative versions of the IPW estimators were discussed by ... Recently, \citet{chen2020doubly} studied properties of the IPW estimators, proved their consistency as well as derived closed form estimators. \citet[section 4.2]{wu2022statistical} argue that $\hat{\mu}_{IPW2}$ estimator performs better than $\hat{\mu}_{IPW1}$ even if the population size is known.

The construction of the IPW estimator involved two steps: 1) estimation of the propensity scores; and 2) deriving $d_i^A$, which in our case are equal to $1/\pi_i^A$. To estimate the propensity scores $\pi_i^A=\pi(\bx_i, \bgamma)$ one can use the likelihood approach under assumption that the information about $\bx_i$ are available for each unit in the population
...
In practice, a function of this form cannot be used because we do not observe all units from the population. More realistic approach involves a reference probability sample $S_B$ and as a result the second component of the \eqref{eq-ipw-loglik} is replaced forming pseudo log-likelihood function 
...
The maximum pseudo-likelihood estimator $\hat{\bgamma}$ can be obtained as the solution to the pseudo score equation which under logit function assumed for $\pi_i^A$ is given by
...
\begin{equation}
\label{gee}
\bG(\bgamma)=\sum_{i \in S_A} h\left(\bx_i, \bgamma\right)-\sum_{i \in S_B} d_i^B \pi\left(\bx_i, \bgamma\right) h\left(\bx_i, \bgamma\right),
\end{equation}

then solving $\bG(\bgamma)=\bZero$ with the chosen parametric form of $\pi_i^A$ and the choosen $\bh(\bx,\bgamma)$ leads to the consistent estimator of $\hat{\bgamma}$.  In the literature, the most commonly considered functions are $h\left(\bx_i, \bgamma\right) = \bx_i$ and $h\left(\bx_i, \bgamma\right) = \bx_i \pi\left(\bx_i, \bgamma\right)^{-1}$.  Note that if the function $h_i=\bx_i$, then $\bG$ is reduced to
...
The IPW and MI estimators are sensible on misspecified models for propensity score and outcome variable respectively. To improve robustness and efficiency, the doubly robust (DR) method incorporates a prediction model for the response variable $Y_i$ and propensity score model for participation $R_i$. This approach is doubly robust in that sense that the DR estimator remains consistent even if one of the models is mis-specified. The general formulae for the for the DR estimator is given by

$$
\tilde{\mu}_{DR} = \frac{1}{N}\sum_{i \in S_A}\frac{y_i - m_i}{\pi_i^A} + \frac{1}{N}\sum_{i =1}^N m_i,
$$
where $\pi_i^A$ and $m_i$ are defined as previously. In the next subsections we discuss two approaches to DR estimation. 

### Joint Randomization Approach

As discussed previously access to population data may be limited that is why we need to consider a joint randomization approach consisting of a non-probability $S_A$ and probability $S_B$ sample. In this approach doubly robust inference is done within the $qp$ or $\xi p$ framework with no specification which one is correct. \citet{chen2020doubly} proposed two DR estimators given in \eqref{eq-dr-known-n} and \eqref{eq-dr-estimated-n} under assumption that population size either known or estimated

\begin{equation}
\hat{\mu}_{DR1}=\frac{1}{N} \sum_{i \in S_{A}} d_i^{A}\left\{y_i-m\left(\bx_i, \hat{\bbeta}\right)\right\}+\frac{1}{N} \sum_{i \in S_{B}} d_i^{B} m\left(\bx_i, \hat{\bbeta}\right),
\label{eq-dr-known-n}
\end{equation}
...
\begin{equation}
\hat{\mu}_{DR2}=\frac{1}{\hat{N}^{A}} \sum_{i \in S_{A}} d_i^{A}\left\{y_i-m\left(\bx_i, \hat{\bbeta}\right)\right\}+\frac{1}{\hat{N}^{B}} \sum_{i \in S_{B}} d_i^{B} m\left(\bx_i, \hat{\bbeta}\right),
...
Estimation of \eqref{eq-dr-known-n} or \eqref{eq-dr-estimated-n} involves estimation of two models separately which is attractive as it is possible to specify different number of variables for the propensity and outcome model. Alternative approach was proposed by \citet{yang_doubly_2020} and will be discussed in the next subsection.


### Minimization of the bias for doubly robust methods

\citet{yang_doubly_2020} discussed variable selection for high-dimensional setting and noted that we cannot control the bias of the estimator, which may increase. Therefore, according to \citet{yang_doubly_2020}, the idea is to determine the equations leading to the estimation of the $\bbeta$ and $\bgamma$ parameters based on the bias of the population mean estimator. In contrast to the joint randomization approach, this method allows for the estimation of the parameters $\bbeta$ and $\bgamma$ in a single step, rather than in two separate steps. First, they derived the bias of the $\hat{\mu}_{DR}$, assuming $\bh(\bx, \bgamma)=\bx$ for the IPW estimator, which is given by equation \eqref{eq-dr-bias}
...
\sum_{i=1}^N \frac{R_i^A}{\pi\left(\bx_i, \bgamma\right)} \frac{\partial m\left(\bx_i, \bbeta\right)}{\partial \bbeta}-\sum_{i \in S_{\mathrm{B}}} d_i^{\mathrm{B}} \frac{\partial m\left(\bx_i, \bbeta\right)}{\partial \bbeta}
\end{array}\right) = \bZero,
\label{eq-bias-min}
\end{equation}

which can be solved using Newton--Raphson optimization method. This method is is equivalent to (kim and haziza) and was extensively discussed by \citet{chen2020doubly} and \citet{wu2022statistical}. The main limitation is the existence of the solution to \eqref{eq-bias-min} as it may not exist unless the two sets of covariates used, respectively, in the outcome regression model and the propensity score model have the same dimensions. That is why \citet{yang_doubly_2020} suggested to use this approach on a union of variables from both models (e.g. after variable selection).

In the \pkg{nonprobsby} package we have implemented this approach not only for $\bh(\bx, \bgamma)=\bx$ but also for $\bh(\bx, \bgamma)=\bx$ and $h\left(\bx_i, \bgamma\right) = \bx_i \pi\left(\bx_i, \bgamma\right)^{-1}$ and various link functions for propensity score model. We also allow selection of this estimation strategy with or without variable selection methods discussed in Section \ref{sec-varsel}. As noted in the beginning the selection between \eqref{eq-dr-known-n} and \eqref{eq-dr-estimated-n} result in a different approach of estimating variance which will be presented in the next subsection.

### Variance estimators for the doubly robust approach

\citet{yang_doubly_2020} derived a closed form estimator for the \eqref{eq-dr-known-n} but this requires knowledge of the population and it is requires bias correction to obtain valid estimator for $V_{\xi p}\left(\hat{\mu}_{DR}-\mu_y\right)$ under the outcome regression model $\xi$. A doubly robust variance estimator for the $\hat{\mu}_{DR2}$ given by \eqref{eq-dr-estimated-n} is not yet available in the literature. In the package, for the analytical variance estimator of the $\hat{\mu}_{DR2}$ we simply replace $N$ with estimated $\hat{N}_A$ and $\hat{N}_B$ and we strongly advice users to be be careful when using this approach. Alternatively, one can use bootstrap approach. \citet{chen2020doubly} showed that bootstrap approach presented in the Section \ref{sec-prediction} have excellent performance when one of the working models is correctly specified. That is why this approach is recommended for all users.
...
$$
\begin{gathered}
\operatorname{Loss}\left(\lambda_\theta\right)=\sum_{j=1}^p\left(\sum_{i=1}^N\left[\frac{R_i^A}{\pi_i^A\left\{\boldsymbol{x}_i^{\mathrm{T}} \hat{\theta}\left(\lambda_\theta\right)\right\}}-\frac{R_i^B}{\pi_i^B}\right] \boldsymbol{x}_{i, j}\right)^2 \\
\operatorname{Loss}\left(\lambda_\beta\right)=\sum_{i=1}^N R_i^A\left[y_i-m\left\{\boldsymbol{x}_i^{\mathrm{T}} \hat{\beta}\left(\lambda_\beta\right)\right\}\right]^2
\end{gathered}
$$
