...
There are several ways of imputing $y_i^*$ and in the package we have implemented the following MI estimators: the semi-parametric approach based on generalized linear models (MI-GLM), kernel smoothing (MI-NPAR), nearest neighbour matching (MI-NN) and predictive mean matching (MI-PMM). 

The properties of the MI-GLM estimator, where $y_i^*$ are imputed with $\hat{m}_i$ from the semi-parametric model, were studied by \citet{kim_combining_2021}. In the \pkg{nonprobsvy} package, we account for the following GLM families: \code{gaussian}, \code{binomial} and \code{poisson}. The MI-NPAR estimator was proposed by \citet{chen_nonparametric_2022} who studied its finite population properties. In the package we implement this approach using local polynomial regression using \code{stats::loess} method. 
...
\item Independently:
  \begin{itemize}
  \item draw a simple random sampling with replacement from the non-probability sample $S_A$ (using \code{base::sample}),
...
E\left\{\frac{1}{N} \sum_{i=1}^N\left\{\frac{R_i^A}{\pi\left(\bx_i, \bgamma\right)} - 1\right\}\left\{y_i-m\left(\bx_i, \bbeta\right)\right\} \right\}\\
& + E\left\{\frac{1}{N} \sum_{i=1}^N\left(R_i^B d_i^B-1\right) m\left(\bx_i, \bbeta\right)\right\}.
\end{aligned}
\label{eq-dr-bias}
\end{equation}

The goal of this approach is to minimize $\operatorname{bias}\left(\hat{\mu}_{D R}\right)^2$, which consists in solving the following system of empirical equations:
...
If a user is interested in viewing the PS estimation results, then access to the \code{"selection"} element is required. This is a \code{list{} of all the results returned by the MLE or GEE estimation method. For example, the evaluation of coefficients can be done in the following way
...
The package contains the main class \code{nonprob} and the supplementary class \code{nonprob_method} and the related \code{nonprob_summary} class. All available \code{S3methods} can be obtained by calling \code{methods(class="nonprobsvy")}. For instance, the \code{check_balance} function, already mentioned in the case study, is used to view the balance by checking how the PS weights reproduce known or estimated population totals; the \code{nobs} function returns the sample size of the probability and non-probability samples:
