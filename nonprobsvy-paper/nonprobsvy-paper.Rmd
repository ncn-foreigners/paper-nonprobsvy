...
The goal of the case study to integrate administrative (\code{admin}) and survey (\code{jvs}) data about job vacancies in Poland.  
...
The calibrated IPW signiciantly improves the balance as can be accessed by the `nonprobsvycheck` function:

```{r}
nonprobsvycheck(~size, ipw_est1)$balance
nonprobsvycheck(~size, ipw_est2)$balance
```

Note that we do not provide a detailed results on the balance as it can be done using existing packages, for instance using the \code{bal.tab} function from the \pkg{cobalt} package \citep{cobalt}.
