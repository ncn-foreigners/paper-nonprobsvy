...
The MI-NN estimator was initially proposed by \citet{rivers2007sampling} under the name \textit{sample matching} and theoretical properties for the MI-NN estimator for large non-probability samples (i.e. $N n_A^{-1}=O(1)$) was studied by \citet{yang2021integration}. The basic idea of the NN matching is as follows: 1) for each $i$ unit in the probability sample $S_B$ find a donor $j$ (or donors) in sample $S_A$ based on some distance between $\bx_i$ and $\bx_j$; 2) use the matched values $y_j$ from $S_A$ to impute missing $y_i$ values in the probability sample $S_B$. The detailed description is presented in the Algorithm \ref{algo-2} in the Appendix.

<!----
skrócić sekcję i dodać odwołania do literatury
- zacząc od MI-GLM i opisac artykuł kima
- opisać MI-NN odnieść się do Riversa, publikcji Yang i naszej
- opisać MI-PMM odniesc sie do publikacji Yang w PMM dla imputacji, naszej o PMM i wariantach
---->
...
<!----
- postać analityczna estymatora wariancji dla MI-GLM
- estymator NN - jak jest duża próba big data, jak mała
- estymator PMM - jak mała i duża
- zaimplementowany bootstrap
---->
