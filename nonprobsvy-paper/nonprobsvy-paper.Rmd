...
Which one were selected (`ncvreg`)
...
The package contains the main class \code{nonprob} and a supplementary class \code{nonprob\_method} and related  \code{nonprob\_summary} class. All available \code{S3methods} can be obtained by calling \code{methods(class="nonprobsvy")}. For instance, the \code{check_balance} function, already mentioned in the case study, is used to view the balance by checking how the PS weights reproduce known or estimated population totals; the \code{nobs} function returns the sample size of the probability and non-probability samples:
...
\code{pop_size} & returns size of the population (fixed or not)\\
...
```{r}
summary(weights(dr_est1), breaks = "fd")
```

For mass imputation estimators we allow to use special functions `method_glm` that are of class `nonprob_method`. An example is given below
...
For the IPW we have created function `method_ps` which returns requested functions for a specific link. The reason for that as follows: 1) we allow different estimation techniques such as maximum likelihood and general estimation equations; 2) IPW estimator is also used for the DR estimator; and 3) variance estimators depends on whether the IPW or the DR estimator is applied (with combination with the MI estimator).
