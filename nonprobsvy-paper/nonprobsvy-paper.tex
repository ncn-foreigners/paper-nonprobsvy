...
dedicated to the PS approach (as of version 0.10.0). It assumes that the
reference probability sample is available and the authors have
...
package \citep{scipy2020}. Neither \pkg{balance} nor \pkg{inps} supports
the use of the \pkg{samplics} package \citep{Diallo2021}. Finally, the
MRP approach is implemented solely in the \pkg{rstanarm} package with
variable selection specified by an appropriate prior.
...
Let
\(\left\{\left(\boldsymbol{x}_i, \pi_{i}^{\text{P}}\right), i \in \mathcal{I}_{\text{P}}\right\}\)
...
\(\pi^{\text{P}}\). Each unit in the \(S_{\text{P}}\) sample has been
assigned a design-based weight given by
\(d_i^{\text{P}} = 1/\pi_i^{\text{P}}\).

Let \(R_i^{\text{NP}}=I(i \in S_{\text{NP}})\) and
\(R_i^{\text{P}}=I(i \in S_{\text{P}})\) be indicators of inclusion in
...
\(\pi_i^{\text{NP}}=P(R_i^{\text{NP}}=1 \mid \boldsymbol{x}_i, y_i)=P(R_i^{\text{NP}}=1 \mid \boldsymbol{x}_i)\)
be propensity scores, which characterize the \(S_{\text{NP}}\) sample's
inclusion and participation mechanisms. Unlike \(\pi_i^{\text{P}}\), the
\(\pi_i^{\text{NP}}\) and \(d_i^{\text{NP}}=1/\pi_i^{\text{NP}}\) are
...
\begin{table}[ht!]
    ...
    \hline
    Sample & ID & Inclusion ($R^{\text{NP}}$) & Design weight ($d$) & Covariates ($\boldsymbol{x}$) & Study variable ($y$) \\
    \hline
...
population totals) to estimate \(\mu_y\). Given the absence of a
...
\begin{itemize}
\item[A1] $R_i^{\text{NP}}$ and the study variable $y_i$ are independent given the set of covariates $\boldsymbol{x}_i$ (the missing at random mechanism).
\item[A2] All the units in the target population have non-zero PS, i.e., $\pi_i^{NP}>0$, $i=1,2, \ldots, N$ (i.e., no coverage error).
\item[A3] The indicator variables $R_1^{\text{NP}}, R_2^{\text{NP}}, \ldots, R_N^{\text{NP}}$ are independent given the set of auxiliary variables $\left(\boldsymbol{x}_1, \boldsymbol{x}_2, \ldots, \boldsymbol{x}_N\right)$ (i.e., no clustering).
\end{itemize}
...
calibrated \(d_i^{\text{P}}\) weights (i.e., \(d_i^{\text{P}}\) adjusted
...
From Equation \ref{eq-pred-two-estimators-simplified} is appealing as it
only requires the non-probability sample \(S_{\text{NP}}\) and reference
population means (or totals and population size \(N\)). If the
population means are unknown, they can be replaced by estimates provided
by the reference probability sample \(S_{\text{P}}\), i.e.,
\(\sum_{i=1}^N \hat{m}_i\) is replaced with
\(\sum_{i \in S_{\text{P}}} d_i^{\text{P}}\hat{m}_i\) for Equation
\ref{eq-pred-two-estimators} and \(\mu_{\boldsymbol{x}}\) is replaced by
\(\hat{\mu}_{\boldsymbol{x}}=\hat{N}_{\text{P}}^{-1}\sum_{i \in S_{\text{P}}}d_i^{\text{P}}\boldsymbol{x}_i\)
...
based on GLM (MI-GLM), kernel smoothing (MI-NPAR), nearest neighbour
...
imputation PMM approach has been proposed to overcome this issue
\citep{chlebicki2025}.
...
\(\pi_i^{\text{NP}}=P\left(i \in S_{\text{NP}} \right)\). As in the case
...
where the \(\hat{\mu}_{y,\text{IPW1}}\) is an adjustment of the
Horvitz-Thompson estimator, and the \(\hat{\mu}_{y,\text{IPW2}}\) is an
adjustment of the Hájek estimator, where the estimated population size
is given by
\(\hat{N}_{\text{NP}} = \sum_{i \in S_{\text{NP}}} (\pi_i^{NP})^{-1}\).
...
\(\hat{\mu}_{y,\text{IPW2}}\) estimator performs better than
\(\hat{\mu}_{y,\text{IPW1}}\) even if the population size is known.
...
\noindent which, in our case, are equal to \(1/\pi_i^{NP}\). To estimate
the propensity scores
\(\pi_i^{NP}=\pi(\boldsymbol{x}_i, \boldsymbol{\gamma})\) one can use
the likelihood approach assuming that the information about
\(\boldsymbol{x}_i\) is available for each unit in the population given
by Equation \ref{eq-ipw-loglik}.

\begin{equation}
\begin{aligned}
\ell(\boldsymbol{\gamma}) &= \log\left\{\prod_{i=1}^N \left(\pi_i^{\text{NP}}\right)^{R_i^{\text{NP}}}\left(1-\pi_i^{\text{NP}}\right)^{1-R_i^{\text{NP}}}\right\} \\
...
\begin{equation}
\ell^*(\boldsymbol{\gamma}) = \sum_{i \in S_{\text{NP}}} \log \left\{\frac{\pi\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right)}{1-\pi\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right)}\right\}+ \sum_{i \in S_{\text{P}}} d_i^{\text{P}} \log \left\{1-\pi\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right)\right\}.
...
under the logit function assumed for \(\pi_i^{NP}\), is given by
Equation \ref{eq-ipw-solution-mle}

\begin{equation}
\boldsymbol{U}(\boldsymbol{\gamma}) = \sum_{i \in S_{\text{NP}}} \boldsymbol{x}_i - \sum_{i \in S_{\text{P}}} d_i^{\text{P}} \pi(\boldsymbol{x}_i, \boldsymbol{\gamma}) \boldsymbol{x}_i.
...
\begin{equation}
\label{gee}
\boldsymbol{G}(\boldsymbol{\gamma})=\sum_{i \in S_{\text{NP}}} \boldsymbol{h}\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right)-\sum_{i \in S_{\text{P}}} d_i^{\text{P}} \pi\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right) \boldsymbol{h}\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right),
\end{equation}

then solving for \(\boldsymbol{G}(\boldsymbol{\gamma})=\boldsymbol{0}\)
with the chosen parametric form of \(\pi_i^{NP}\) and the chosen
...
\begin{equation}
\boldsymbol{G}(\boldsymbol{\gamma}) = \sum_{i \in S_{\text{NP}}} \frac{\boldsymbol{x}_i}{\pi\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right) }-\sum_{i \in S_{\text{P}}} d_i^{\text{P}} \boldsymbol{x}_i,
...
\(\hat{\mu}_{y,\text{IPW2}}\) estimator assuming logistic regression. In
the package we have implemented this approach for \code{"logit"},
\code{"probit"} and \code{"cloglog"} link functions of the
\code{binomial()} family. We refer the reader to
...
\(\hat{\mu}_y\) is replaced by one of the estimators in Equation
...
\(R_i^{\text{NP}}\). This approach is doubly robust in the sense that
...
where \(\pi_i^{\text{NP}}\) and \(m_i\) are defined as previously. In
...
\begin{equation}
\hat{\mu}_{y,\text{DR1}}=\frac{1}{N} \sum_{i \in S_{\text{NP}}} d_i^{\text{NP}}\left\{y_i-m\left(\boldsymbol{x}_i, \hat{\boldsymbol{\beta}}\right)\right\}+\frac{1}{N} \sum_{i \in S_{\text{P}}} d_i^{\text{P}} m\left(\boldsymbol{x}_i, \hat{\boldsymbol{\beta}}\right),
\label{eq-dr-known-n}
\end{equation}
...
\begin{equation}
\hat{\mu}_{y,\text{DR2}}=\frac{1}{\hat{N}_{\text{NP}}} \sum_{i \in S_{\text{NP}}} d_i^{\text{NP}}\left\{y_i-m\left(\boldsymbol{x}_i, \hat{\boldsymbol{\beta}}\right)\right\}+\frac{1}{\hat{N}_{\text{P}}} \sum_{i \in S_{\text{P}}} d_i^{\text{P}} m\left(\boldsymbol{x}_i, \hat{\boldsymbol{\beta}}\right),
\label{eq-dr-estimated-n}
\end{equation}

where
\(d_i^{\text{NP}}=\pi\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right)^{-1}, \hat{N}_{\text{NP}}=\sum_{i \in S_{\text{NP}}} d_i^{\text{NP}}\)
and \(\hat{N}_{\text{P}}=\sum_{i \in S_{\text{P}}} d_i^{\text{P}}\). The
...
E\left\{\frac{1}{N} \sum_{i=1}^N\left\{\frac{R_i^{\text{NP}}}{\pi\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right)} - 1\right\}\left\{y_i-m\left(\boldsymbol{x}_i, \boldsymbol{\beta}\right)\right\} \right\}\\
& + E\left\{\frac{1}{N} \sum_{i=1}^N\left(R_i^{\text{P}} d_i^{\text{P}}-1\right) m\left(\boldsymbol{x}_i, \boldsymbol{\beta}\right)\right\}.
\end{aligned}
\label{eq-dr-bias}
\end{equation}
...
\begin{equation}
\left(\begin{array}{c}
\sum_{i=1}^N R_i^{\text{NP}}\left\{\frac{1}{\pi\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right)}-1\right\}\left\{y_i-m\left(\boldsymbol{x}_i, \boldsymbol{\beta}\right)\right\} \boldsymbol{x}_i \\
\sum_{i=1}^N \frac{R_i^{\text{NP}}}{\pi\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right)} \dot{m}\left(\boldsymbol{x}_i, \boldsymbol{\beta}\right) -\sum_{i \in S_{\text{P}}} d_i^{\text{P}} \dot{m}\left(\boldsymbol{x}_i, \boldsymbol{\beta}\right)
...
\(\hat{\mu}_{y,\text{DR2}}\) given by Equation \ref{eq-dr-estimated-n}
is not yet available in the literature. In the package, to offer the
analytical variance estimator of \(\hat{\mu}_{y,\text{DR2}}\) we simply
replace \(N\) with estimated \(\hat{N}_{\text{NP}}\) and
\(\hat{N}_{\text{P}}\) but we urge caution when using this approach.
...
\begin{equation}  
\operatorname{Loss}\left(\lambda_{\boldsymbol{\beta}}\right)=\sum_{i=1}^N R_i^{\text{NP}}\left[y_i-m\left\{\boldsymbol{x}_i, \boldsymbol{\beta}(\lambda_{\boldsymbol{\beta}})\right\}\right]^2,
...
\hline
$\boldsymbol{x}_i$ & $\sum_{l=1}^L\left(\sum_{i=1}^N\left[R_i^{\text{NP}} - \frac{R_i^{\text{P}}\pi\left\{\boldsymbol{x}_i, \boldsymbol{\gamma}(\lambda_{\boldsymbol{\gamma}})\right\}}{\pi_i^{\text{P}}}\right] \boldsymbol{x}_{i, l}\right)^2$\\
\hline
$\boldsymbol{x}_i \pi(\boldsymbol{x}_i, \boldsymbol{\gamma})^{-1}$ & $\sum_{l=1}^L\left(\sum_{i=1}^N\left[\frac{R_i^{\text{NP}}}{\pi\left\{\boldsymbol{x}_i, \boldsymbol{\gamma}(\lambda_{\boldsymbol{\gamma}})\right\}}-\frac{R_i^{\text{P}}}{\pi_i^{\text{P}}}\right] \boldsymbol{x}_{i, l}\right)^2$ \\
\hline
\end{tabular}
...
to select other link functions for the \(\pi_i^{\text{NP}}\),
...
the NN and PMM estimators, we employ \(k=5\) nearest neighbours (i.e.,
\texttt{control\_out(k=5)}) but the variance of the PMM estimator should
...
approximation defined in \texttt{control\_out(eps)} may give slightly
...
\texttt{family\_outcome} argument as no model is estimated underneath.
...
using the generic \code{confint} method.
...
weights, we suggest using the \code{weights} method.
...
\State If $k>1$, then
$$\hat{\nu}(i, z) = \operatornamewithlimits{arg\,min}_{\displaystyle j\in S_{\text{NP}}\setminus\bigcup_{t=1}^{z-1}
