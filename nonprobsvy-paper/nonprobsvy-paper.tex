...
The package contains the main class \code{nonprob} and a supplementary
class \code{nonprob\_method} and related \code{nonprob\_summary} class.
All available \code{S3methods} can be obtained by calling
...
For mass imputation estimators we allow to use special functions
\texttt{method\_glm} that are of class \texttt{nonprob\_method}. An
...
For the IPW we have created function \texttt{method\_ps} which returns
requested functions for a specific link. The reason for that as follows:
1) we allow different estimation techniques such as maximum likelihood
and general estimation equations; 2) IPW estimator is also used for the
DR estimator; and 3) variance estimators depends on whether the IPW or
the DR estimator is applied (with combination with the MI estimator).
