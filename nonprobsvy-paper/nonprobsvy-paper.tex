...
which can be viewed as calibrated IPW. Moreover, using
\eqref{eq-ipw-solution-gee} requires the knowledge of population totals
for the auxiliary variables \(\boldsymbol{x}\).
...
Another approach is to use bootstrap which esentially is the same as the
one presented in the \eqref{eq-var-bootstrap} where the \(\hat{\mu}_y\)
is replaced by one of the \eqref{eq-ipw-estimators} estimators.
...
single_shift1 0.7083228 0.010114352
\end{CodeOutput}
\end{CodeChunk}
