...
on non-probability samples: \pkg{NonProbEst} \citep{NonProbEst},
\pkg{balance} \citep{sarig2023balancepythonpackage}, \pkg{inps}
\citep{castro2024inps} and our \pkg{nonprobsvy}. We have included two
packages that implements specific methods: \pkg{rstanarm} (MRP;
...
The \pkg{NonProbEst} implements IPW under logistic regression as well as
...
The \pkg{balance} focuses solely on the IPW and uses standard and
...
To our knowledge the \pkg{nonprobsvy} is the solely software (open or
...
\(\{\left(\boldsymbol{x}_i, \pi_{i}^B\right), i \in S_B\}\) be a dataset
of a probability sample \(S_B\) of size \(n_B\), where only information
about variables \(\boldsymbol{X}\) and inclusion probabilities \(\pi^B\)
are known for all units in the population. Each unit in the sample
\(S_B\) has been assigned a design-based weight given by
\(d_i^B = 1/\pi_i^B\).

Let \(R_i=I(i \in S_A)\) be an indicator of inclusion into
non-probability sample \(S_A\) defined for all units in the target
population. Let
\(\pi_i^A=P(R_i=1 \mid \boldsymbol{x}_i, y_i)=P(R_i=1 \mid \boldsymbol{x}_i)\)
be the propensity scores which characterize the sample \(S_A\) inclusion
and participation mechanisms. On the contrary to \(\pi_i^B\), the
\(\pi_i^A\) and thus \(d_i^A=1/\pi_i^A\) are unknown. This description
...
\begin{itemize}
\item[A1] $R_i$ and the study variable $y_i$ are independent given the set of covariates $\boldsymbol{x}_i$, i.e., $\left(R_i \perp y_i\right) \mid \boldsymbol{x}_i$.
\item[A2] All the units in the target population have non-zero propensity scores, i.e., $\pi_i^A>0$, $i=1,2, \ldots, N$.
\item[A3] The indicator variables $R_1, R_2, \ldots, R_N$ are independent given the set of auxiliary variables $\left(\boldsymbol{x}_1, \boldsymbol{x}_2, \ldots, \boldsymbol{x}_N\right)$.
\end{itemize}
...
cf.~\citet{sarndal2005estimation}) but this requires additional
developments in the theory about the consistency of the MI, IPW or DR
...
In prediction approach the following semiparametric model for the finite
population, denoted as \(\xi\) is assumed
...
such as generalized linear models. Let \(\boldsymbol{\beta}_0\) and
\(\sigma^2_0\) be the true values of the model parameters
\(\boldsymbol{\beta}\) and \(\sigma^2_0\) under assumed model and the
...
\begin{equation}
\hat{\mu}_{y 1}=\mu_{\boldsymbol{x}}^{\prime} \hat{\boldsymbol{\beta}} \quad \text { and } \quad \hat{\mu}_{y 2}=\frac{n_A}{N}\left(\bar{y}_A-\overline{\boldsymbol{x}}_A^{\prime} \hat{\boldsymbol{\beta}}\right)+\mu_{\boldsymbol{x}}^{\prime} \hat{\boldsymbol{\beta}},
...
and \(\overline{\boldsymbol{x}}_A=n_A\sum_{i \in S_A}\boldsymbol{x}_i\)
...
estimator then \(\hat{\mu}_{y 1}=\hat{\mu}_{y 2}\).

This form is appealing as the only requirement is availability of
...
\(S_B\): \(\sum_{i=1}^N \hat{m}_i = \sum_{i \in S_B} d_i^B\hat{m}_i\)
for \eqref{eq-pred-two-estimators} and \(\mu_{\boldsymbol{x}}\) replaced
by
...
Model-based prediction estimators of \(\mu_y\) can be treated as
...
the non-probability sample \(S_A\) as \((y_i, \boldsymbol{x}_i)\) is
observed. The general for of the mass imputation (MI) estimator is given
by
...
where \(y_i^*\) is the imputed \(y_i\) value and \(\hat{N}_{B}\) is
defined previously. Under deterministic regression imputation the
...
based on the generalized linear models (hereinafter denoted as MI-GLM),
nearest neighbour matching (denoted as MI-NN) and predictive mean
matching (MI-PMM).

The properties of the MI-GLM estimator was studied by
\citet{kim_combining_2021} additional assumption called
\textit{model transportability}: the model which is built for the
non-probability sample \(S_A\) can be used for prediction with the
reference probability sample \(S_B\). In the package we allow for the
following GLM families: \code{gaussian}, \code{binomial} and
\code{poisson}.
...
the MI-NN estimator for large non-probability samples
...
over a vector of selected \(y\) is calculated. The detailed description
is presented in the Algorithm \ref{algo-2} in the Appendix. This
algorithm suffers from the curse of dimensionality as proved by for
i.e.~asymptotic bias of the MI estimator increases as the number of
covariates \(\boldsymbol{x}\) increases with a fixed \(k\). To overcome
this issue PMM was proposed

In the PMM matching is done using predicted values of
...
\(S_B\) to donor units from sample \(S_A\). The remaining part is the
same as in the NN approach. \citet{chlebicki2025} studied properties of
two variants of the MI-PMM estimator for non-probability samples:
matching predicted to predicted (\(\hat{m}-\hat{m}\) matching; MI-PMM A)
and predicted to observed (\(\hat{m}-y\) matching; PMM B). Details about
the procedure can be found in Algorithm \ref{algo-3} and \ref{algo-4} in
the Appendix. \citet{chlebicki2025} prove consistency of the MI-PMM A
estimator under model mis-specification i.e.~the assumed model may be
...
contains two components: \(\hat{V}_1\) (based on information from both
\(S_A\) and \(S_B\)) and \(\hat{V}_2\) (based solely on probability
sample \(S_B\)); for the MI-NN estimator for large samples
...
the mean) and for small \(S_A\) samples variance estimator can be found
in \citet{chlebicki2025}; and for the MI-PMM estimators
\citet{chlebicki2025} contains formulas for variance estimators for
small and large non-probability samples \(S_A\).

Second approach is based on the bootstrap where each run of the
procedure \(b=1,...,B\) can be expresed as follows
...
After obtaining \(B\) bootstrap estimate estimate variance based on the
following equation
...
where \(\hat{\mu}_{y}\) is the estimated mean using either MI-GLM, MI-NN
or MI-PMM estimator.
...
use the first component \(\hat{V}_1\) of the \citet{kim_combining_2021}
variance estimator (estimated population quantities are replaced with
known ones). For the variance of the MI-NN and MI-PMM estimators we only
use bootstrap approach with known population means. Note, that current
...
involves estimation of propensity scores denoted as
\(\pi_i^A=P\left(i \in S_A \right)\) which denotes inclusion and
participation probabilities. Similarly as for the prediction approach we
have two variants of the IPW estimators given by
...
\(\hat{N}^A = \sum_{i \in S_A} \frac{1}{\pi_i^A}\). This estimator in
the case of non-probability samples was discussed by .. and several
approaches on using propensity scores and alternative versions of the
IPW estimators were discussed by \ldots{} Recently,
\citet{chen2020doubly} studied properties of the IPW estimators, proved
their consistency as well as derived closed form estimators.
...
of the propensity scores; and 2) deriving \(d_i^A\), which in our case
are equal to \(1/\pi_i^A\). To estimate the propensity scores
...
\(\boldsymbol{x}_i\) are available for each unit in the population
...
\begin{equation}
\label{gee}
\boldsymbol{G}(\boldsymbol{\gamma})=\sum_{i \in S_A} h\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right)-\sum_{i \in S_B} d_i^B \pi\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right) h\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right),
\end{equation}

then solving \(\boldsymbol{G}(\boldsymbol{\gamma})=\boldsymbol{0}\) with
the chosen parametric form of \(\pi_i^A\) and the choosen
...
\(h\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right) = \boldsymbol{x}_i\)
and
\(h\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right) = \boldsymbol{x}_i \pi\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right)^{-1}\).
Note that if the function \(h_i=\boldsymbol{x}_i\), then
...
If we use the second form of the function \(h\) we get the following
form of the function \(\mathbf{G}\)

\begin{equation}
\mathbf{G}(\boldsymbol{\theta}) = \sum_{i \in S_A} \frac{\boldsymbol{x}_i}{\pi\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right) }-\sum_{i \in S_B} d_i^B \boldsymbol{x}_i,
\label{eq-ipw-solution-gee}
\end{equation}

which can be viewed as calibrated IPW and equation
...
the outcome model is linear (\ldots{} riddles survey meth).
...
propensity score and outcome variable respectively. To improve
robustness and efficiency, the doubly robust (DR) method incorporates a
prediction model for the response variable \(Y_i\) and propensity score
model for participation \(R_i\). This approach is doubly robust in that
sense that the DR estimator remains consistent even if one of the models
is mis-specified. The general formulae for the for the DR estimator is
given by
...
As discussed previously access to population data may be limited that is
why we need to consider a joint randomization approach consisting of a
non-probability \(S_A\) and probability \(S_B\) sample. In this approach
doubly robust inference is done within the \(qp\) or \(\xi p\) framework
with no specification which one is correct. \citet{chen2020doubly}
proposed two DR estimators given in \eqref{eq-dr-known-n} and
\eqref{eq-dr-estimated-n} under assumption that population size either
known or estimated
...
Estimation of \eqref{eq-dr-known-n} or \eqref{eq-dr-estimated-n}
involves estimation of two models separately which is attractive as it
is possible to specify different number of variables for the propensity
and outcome model. Alternative approach was proposed by
\citet{yang_doubly_2020} and will be discussed in the next subsection.
...
mean estimator. In contrast to the joint randomization approach, this
method allows for the estimation of the parameters
...
\sum_{i=1}^N \frac{R_i^A}{\pi\left(\boldsymbol{x}_i, \boldsymbol{\gamma}\right)} \frac{\partial m\left(\boldsymbol{x}_i, \boldsymbol{\beta}\right)}{\partial \boldsymbol{\beta}}-\sum_{i \in S_{\mathrm{B}}} d_i^{\mathrm{B}} \frac{\partial m\left(\boldsymbol{x}_i, \boldsymbol{\beta}\right)}{\partial \boldsymbol{\beta}}
\end{array}\right) = \boldsymbol{0},
\label{eq-bias-min}
\end{equation}

which can be solved using Newton--Raphson optimization method. This
method is is equivalent to (kim and haziza) and was extensively
discussed by \citet{chen2020doubly} and \citet{wu2022statistical}. The
main limitation is the existence of the solution to \eqref{eq-bias-min}
as it may not exist unless the two sets of covariates used,
respectively, in the outcome regression model and the propensity score
model have the same dimensions. That is why \citet{yang_doubly_2020}
suggested to use this approach on a union of variables from both models
(e.g.~after variable selection).

In the \pkg{nonprobsby} package we have implemented this approach not
only for
\(\boldsymbol{h}(\boldsymbol{x}, \boldsymbol{\gamma})=\boldsymbol{x}\)
but also for
\(\boldsymbol{h}(\boldsymbol{x}, \boldsymbol{\gamma})=\boldsymbol{x}\)
...
to be be careful when using this approach. Alternatively, one can use
bootstrap approach. \citet{chen2020doubly} showed that bootstrap
approach presented in the Section \ref{sec-prediction} have excellent
performance when one of the working models is correctly specified. That
...
single_shift1 0.7083228 0.010972976
\end{CodeOutput}
\end{CodeChunk}
