...
proposed by \citet{Schonlau2017}. The \pkg{inps} supports the use of
...
included in the \pkg{survey} package to account for the design of the
probability sample. Finally, we provide a user-friendly API that mimics
\code{glm} or other functions known in \proglang{R}, together with a key
...
hold for all units in the non-probability \(S_A\) sample. The parameters
of the \eqref{eq-semipar-model} can be estimated using the quasi maximum
...
calculated for all units \(i=1,...,N\). Under this settings, as
...
\citet[section 4.2]{wu2022statistical} argue that the
...
that the second component of the \eqref{eq-ipw-loglik} is replaced,
yielding a pseudo log-likelihood function given by
...
models estimated separately, which is an attractive option, since once
...
estimated \(\hat{N}_A\) and \(\hat{N}_B\) and we urge caution when using
this approach.
...
IPW bootstrap variance 0.7083228 0.011179381
\end{CodeOutput}
\end{CodeChunk}
...
software for addressing inference challenges with non-probability
...
leveraging auxiliary data. By providing a unified framework and
integration with the \pkg{survey} package, the \pkg{nonprobsvy} makes
...
selections mechanism, developing theory for maintaining consistency with
calibration weights, and supporting multiple non-probability samples for
data integration from various sources. Further methodological extensions
under consideration include empirical likelihood approaches for
doubly/multiply robust estimation, integration of machine learning
methods like debiased/double machine learning from causal inference,
handling measurement error in big data variables, and expanding the
bootstrap approach beyond simple random sampling with replacement.

The package will also be extended to work with the
...
the package, testing, reviewing and contributing to the source code and
