...
\begin{CodeChunk}
\begin{CodeInput}
R> nonprobsvycheck(~size, ipw_est1)$balance
\end{CodeInput}
\begin{CodeOutput}
    sizeL     sizeM     sizeS 
-367.6153 -228.3993 1624.1408 
\end{CodeOutput}
\begin{CodeInput}
R> nonprobsvycheck(~size, ipw_est2)$balance
\end{CodeInput}
\begin{CodeOutput}
 sizeL  sizeM  sizeS 
 3e-10  0e+00 -4e-10 
...
Note that we do not provide a detailed results on the balance as it can
be done using existing packages, for instance using the \code{bal.code}
function from the \pkg{cobalt} package \citep{cobalt}.
