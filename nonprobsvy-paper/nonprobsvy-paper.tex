...
In addition, we assume no overlap between samples and no measurement
error in \(Y_i\) and \(\boldsymbol{X}_i\) is observed. Setting presented
in Table \ref{tab-two-sources} may be also extended to calibrated
\(d_i\) weights (i.e.~\(d_i\) adjusted for under-coverage, non-contact
or non-response; cf.~\citet{sarndal2007calibration}) but this requires
...
As \citet{wu2022statistical} notes there are two commonly used
...
As presented in Table \ref{tab-two-sources}, we do not know the value of
...
single_shift1 0.7083228 0.010525966
\end{CodeOutput}
\end{CodeChunk}
