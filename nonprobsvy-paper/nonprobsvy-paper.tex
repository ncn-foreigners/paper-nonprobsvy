...
\Abstract{
The following paper presents \pkg{nonprobsvy} -- an \proglang{R} package
...
60-624 Poznań, Poland\\
  ...
  URL: \url{https://berenz.github.io/}\\~\\
  }
...
\citet[eg. chapter 8]{lohr2021sas} who covers \proglang{SAS} software
...
does not allow users to leverage the \pkg{survey} package for
estimation. The \pkg{balance} package is solely dedicated to the PS
...
\citet{Schonlau2017}. The \pkg{inps} package supports the use of
...
\pkg{scipy.stats} module \citep{scipy2020}. Neither \pkg{balance} nor
\pkg{ips} supports the use of the \pkg{samplics} module
\citep{Diallo2021}. Finally, the MRP approach is implemented solely in
the \pkg{rstanarm} package with variable selection specified by an
appropriate prior.
...
user-friendly API that mimics \code{glm}, \code{survey::svydesign} and
other functions known in \proglang{R}, together with the main function
...
regression using \code{stats::loess} method. In the next releases we
