...
obligatory ones are \code{data}, while \code{selection}, \code{outcome},
or \code{target} must be specified depending on the chosen method.
...
In this chapter, we will show you how to use the main \code{nonprob}
...
nonprobsvy package is to define an object using the \code{svydesign}
function that stores the probability sample \code{data.frame} and other
...
documentation), \pkg{nleqslv} \cite{nleqslv} (solving non-linear
equations), and \pkg{doParallel} \citep{doParallel} (parallel
computing).
...
argument. This part is implemented in the \pkg{Rcpp} and
...
single_shift1 0.7083228 0.009994811
\end{CodeOutput}
\end{CodeChunk}
